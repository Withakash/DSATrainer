  ...
  return (
    <button
      ...
      className={`rounded-md px-3 py-1.5 text-sm font-medium transition disabled:cursor-not-allowed disabled:opacity-40 ${
        primary ? "bg-emerald-600 text-white hover:bg-emerald-500" : "border border-neutral-700 text-neutral-300 hover:bg-neutral-800"
      }`}
    >
      {children}
    </button>
...
// Play / Pause / Restart / Prev / Next + speed. Reuses the shared playback hook.
      ...
      <div className="ml-auto flex items-center gap-1 rounded-md border border-neutral-800 bg-neutral-900/40 p-0.5">
        {SPEEDS.map((s) => (
          <button
            ...
            className={`rounded px-2 py-1 text-xs font-medium transition ${pb.speed === s ? "bg-emerald-600 text-white" : "text-neutral-400 hover:bg-neutral-800"}`}
          >
            {s}x
          </button>
        ))}
