import type { SQCategory } from "@/lib/visualizer/stack-queue/stackQueueTypes";

const CATEGORY_LABEL: Record<SQCategory, string> = {
  stack: "Stack (LIFO)",
  queue: "Queue (FIFO)",
...
// Persistent learning context: pattern, key insight, where it shows up.
  ...
  return (
    <section className="rounded-lg border border-indigo-800 bg-indigo-950/20 p-4">
      <div className="flex flex-wrap items-center gap-2">
        <span className="rounded-full border border-indigo-700 bg-indigo-950/40 px-2.5 py-0.5 text-xs font-semibold text-indigo-300">{CATEGORY_LABEL[category]}</span>
      </div>
      {keyIdea && (
