...
// POST /api/solve  Body: { problem: string }  →  { data: SolverResult }
...
export async function POST(request: Request) {
  ...
  console.log(`[/api/solve] request (problem ${problem.length} chars)`);
  try {
    const data = await aiRouter.solve(problem);
    return NextResponse.json({ data });
  } catch (error) {
    const pe = error instanceof ProviderError ? error : null;
    const kind = pe?.kind ?? "unknown";
    const detail = pe?.details ? ` (${pe.details})` : "";
    console.error("[/api/solve] failed:", kind, pe?.details ?? (error instanceof Error ? error.message : error));
